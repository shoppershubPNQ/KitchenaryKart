 ...
 *
 * ⚠️ DUPLICATED from admin/lib/invoice.ts so the customer-facing
 * "Download Tax Invoice" on the storefront produces the SAME PDF the
 * admin generates. Keep both in sync (like shipping-zones.ts).
 *
...
import type { OrderSummary } from './invoice-summary';
